    ...
    if (name in data) == False:
        ...
        for p in settings["rates"]:
            ...
            for n in range(len(markets)):
                if ((markets[n][0] == a) and (markets[n][1] == b)):
                    marketid = markets[n][2]
            try:
                adress = "http://pubapi.cryptsy.com/api.php?method=singlemarketdata&marketid=%s" % marketid
            ...
            timestamp = cryptsy["return"]["markets"][a]["lasttradetime"]
            data[name][a][b]["timestamp"] = datetime.datetime.strptime(timestamp,"%Y-%m-%d %H:%M:%S")
            data[name][a][b]["timestamp"] = data[name][a][b]["timestamp"] + datetime.timedelta(hours=5)
            data[name][a][b]["timestamp"] = data[name][a][b]["timestamp"].replace(tzinfo=crypttime)
            data[name][a][b]["timestamp"] = datetime.datetime.fromtimestamp(calendar.timegm(data[name][a][b]["timestamp"].timetuple()))
...
def display( exchange_name , data, simple ):
    ...
    for f in data[exchange_name]:
        for v in data[exchange_name][f]:
            ...
            print "\tLast: \t%.8f" % (data[exchange_name][f][v].get("last",0))
